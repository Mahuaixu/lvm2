				   ...
				   struct dm_list *add_cache_devs)
{
	...
	/*
	 * When there are duplicate PVs, there is a chance that the devices
	 * represent different underlying storage, and there is a chance that
	 * we choose to prefer the wrong device here.  Because of this we
	 * should prevent writing the PV/VG while the correct device is
	 * uncertain.
	 *
	 * PERMITTED_READ_ONLY commands may write metadata to fix it in vg_read
	 * if it looks inconsistent.  For these commands, set
	 * metadata_read_only to prevent mistakenly "fixing" an incorrect PV.
	 *
	 * Non-PERMITTED_READ_ONLY commands expect to write metadata.
	 * strict_pv_device=1 prevents these commands by setting
	 * metadata_read_only.
	 * strict_pv_device=0 allows these commands because this is the
	 * old behavior that may still be needed in some cases.
	 *
	 * FIXME: also prevent lv activation when strict_pv_device=1?
	 */

	if (cmd->permitted_read_only)
...
next:
	...
	if (!(info = lvmcache_info_from_pvid(alt->dev->pvid, 0))) {
		...
		log_warn("PV %s on duplicate device %s not found in cache.",
	...
	dm_list_iterate_items(devl, &altdevs) {
		...
		has_holders1 = dm_device_has_holders(dev1_major, dev1_minor);
		has_holders2 = dm_device_has_holders(dev2_major, dev2_minor);
		...
		log_warn("PV %s compare duplicates %s and %s",
		...
		if (has_fs1 && !has_fs2) {
			...
			/* change to 2 */
			change = 1;
		} else if (has_holders1 && !has_holders2) {
			/* keep 1 */
		} else if (has_holders2 && !has_holders1) {
			/* change to 2 */
			change = 1;
	...
	if (dev1 != info->dev) {
		log_warn("PV %s switch from %s to preferred %s",
			 info->dev->pvid, dev_name(info->dev), dev_name(dev1));
...
int lvmcache_label_scan(struct cmd_context *cmd)
{
	...
	if (!dm_list_empty(&_found_duplicate_devs)) {
		...
		dm_list_iterate_items(devl, &del_cache_devs) {
			log_warn("Drop duplicate device %s in lvmcache", dev_name(devl->dev));
			if ((info = lvmcache_info_from_pvid(devl->dev->pvid, 0)))
				lvmcache_del(info);
		}

		dm_list_iterate_items(devl, &add_cache_devs) {
			log_warn("Rescan preferred device %s for lvmcache", dev_name(devl->dev));
			(void) label_read(devl->dev, &label, UINT64_C(0));
		}
				   ...
				   const char *vgname, const char *vgid, uint32_t vgstatus)
{
	...
	if (!created) {
		if (info->dev != dev) {
			log_warn("PV %s on %s was already found on %s.",
				  pvid_s, dev_name(dev), dev_name(info->dev));
