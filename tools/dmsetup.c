...
static int _stats_group(CMD_ARGS)
{
	...
	int aggregate = _switches[AGGREGATE_ARG];
	struct dm_stats *dms;
	uint64_t group_id;
	int r = 0;
	...
	if(!dm_stats_create_group(dms, regions, alias, aggregate, &group_id)) {
		log_error("Could not create group on %s: %s", name, regions);
		goto out;
	}

	log_info("Created new group %s as group ID " FMTu64,
		 regions, group_id);
...
static int _stats_ungroup(CMD_ARGS)
{
	...
	if (!(r = dm_stats_delete_group(dms, group_id)))
		log_error("Could not delete group " FMTu64 " on %s.",
			  group_id, name);

	log_info("Deleted group ID "FMTu64" on %s", group_id, name);
...
#define GROUP_OPTS "[--aggregate] [--alias NAME] --members <regions>"
...
static int _process_switches(int *argcp, char ***argvp, const char *dev_dir)
{
	...
	while ((ind = -1, c = GETOPTLONG_FN(*argcp, *argvp, "cCfG:hj:m:M:no:O:rS:u:U:vy",
					    long_options, NULL)) != -1) {
		if (ind == AGGREGATE_ARG)
			_switches[AGGREGATE_ARG]++;
		if (ind == ALIAS_ARG) {
			_switches[ALIAS_ARG]++;
